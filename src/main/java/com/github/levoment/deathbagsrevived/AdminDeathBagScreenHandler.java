...
public class AdminDeathBagScreenHandler extends ScreenHandler {
    ...
    private int currentInventoryInQueue;
    ...
    public AdminDeathBagScreenHandler(int syncId, PlayerInventory playerInventory, Text playerName) {
        this(syncId, playerInventory, new PlayerInventory(playerInventory.player));
        this.playerName = playerName.getString();
    }
    public AdminDeathBagScreenHandler(int syncId, PlayerInventory playerInventory) {
        this(syncId, playerInventory, new PlayerInventory(playerInventory.player));
    }

    public AdminDeathBagScreenHandler(int syncId, PlayerInventory playerInventory, Inventory inventory) {
        ...
        ItemStack activeItem = this.playerInventory.player.getMainHandStack();
        if (activeItem.isEmpty()) activeItem = this.playerInventory.player.getOffHandStack();
        if (activeItem.getItem() instanceof AdminDeathBagItem adminDeathBagItem) {
            this.playerName = adminDeathBagItem.getPlayerName();
            this.currentInventoryInQueue = adminDeathBagItem.getLastAccessedInventory();
            this.currentItem = activeItem;
            this.playerUUID = adminDeathBagItem.getPlayerUUID();
        };
    ...
    public void close(PlayerEntity player) {
        if (player instanceof ServerPlayerEntity) {
                ...
                if (deathBagsRevivedFolder.exists() && deathBagsRevivedFolder.canWrite()) {
                    ...
                    if (allPlayersInventoriesFile.exists()) {
                        try {
                            ...
                            NbtList playerInventories = previousData.getList(this.playerUUID.toString(), NbtElement.LIST_TYPE);
                            // Remove the last list if the size of inventory is more than 0
                            if (this.currentInventoryInQueue >= 0 && this.currentInventoryInQueue < playerInventories.size()) playerInventories.remove(this.currentInventoryInQueue);
                            previousData.put(this.playerUUID.toString(), playerInventories);
                            // Write to file
                            NbtIo.write(previousData, allPlayersInventoriesFile);
                            // Get rid of the item because it is empty if it's not a GoldenDeathBag
                            for (int i = 0; i < this.playerInventory.size(); i++) {
            ...
            if (!deathBagInventory.isEmpty() && player instanceof ServerPlayerEntity) {
                    ...
                    if (allPlayersInventoriesFile.exists()) {
                        try {
                            ...
                            if (playerInventories.size() > 0) {
                                List<NbtList> temporaryList = new ArrayList<>();
                                for (int i = 0; i < playerInventories.size(); i++) {
                                        temporaryList.add(playerInventories.getList(i));
                                }
                                playerInventories = new NbtList();
                                for (int i = 0; i < temporaryList.size(); i++) {
                                    if (i == this.currentInventoryInQueue) {
                                        playerInventories.add(i, list);
                                    } else {
                                        playerInventories.add(i, temporaryList.get(i));
                                    }
                                }
                            }
                            previousData.remove(this.playerUUID.toString());
                            previousData.put(this.playerUUID.toString(), playerInventories);
                            // Write to file
                            NbtIo.write(previousData, allPlayersInventoriesFile);
...
//                            NbtIo.write(previousData, allPlayersInventoriesFile);
                        ...
                        nbtCompound.put(this.playerUUID.toString(), rootList);
                        try {
        ...
        this.currentInventoryInQueue--;
        if (this.currentItem != null) {
            if (this.currentItem.getItem() instanceof AdminDeathBagItem adminDeathBagItem) {
                adminDeathBagItem.setPlayerName(this.playerName);
                adminDeathBagItem.setLastAccessedInventory(currentInventoryInQueue);
                adminDeathBagItem.setPlayerUUID(this.playerUUID);
            }
        }
        super.close(player);
    }
        ...
        if (this.playerInventory.player.getServer() != null) {
            ...
            if (allPlayersInventoriesFile.exists()) {
                try {
                    ...
                    NbtList playerInventories = previousData.getList(this.playerUUID.toString(), NbtElement.LIST_TYPE);
                    NbtList lastList;
                    if (this.currentInventoryInQueue == -1) {
                        this.currentInventoryInQueue = playerInventories.size() - 1;
                        // Get the last player inventory
                        lastList = playerInventories.getList(this.currentInventoryInQueue);
                    } else {
                        lastList = playerInventories.getList(this.currentInventoryInQueue);
                    }
    ...
    public int getCurrentInventoryInQueue() {
        return currentInventoryInQueue;
    }

    public void setCurrentInventoryInQueue(int currentInventoryInQueue) {
        this.currentInventoryInQueue = currentInventoryInQueue;
    }
}
